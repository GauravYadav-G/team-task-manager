...
export default function TaskCard({ task, onEdit, onStatusChange, userRole }) {
  ...
  const priorityConfig = useMemo(() => {
    switch (task.priority) {
      ...
      case 'MEDIUM':
        return { border: 'border-l-4 border-l-orange-500', text: 'text-orange-500', glow: 'shadow-orange-500/5' };
      case 'LOW':
      default:
  ...
  return (
    <div
      ...
      onClick={() => onEdit?.(task)}
      id={`task-${task.id}`}
      className={`group bg-[#1E1E1E] hover:bg-[#262626] border border-white/5 hover:border-white/10 p-5 rounded-2xl cursor-grab active:cursor-grabbing transition-all duration-300 hover:shadow-lg ${priorityConfig.border} ${priorityConfig.glow} ${overdue ? 'bg-red-500/5 border-red-500/20' : ''}`}
    >
      ...
      <h4 className="font-extrabold text-sm text-[#FDFBF7] group-hover:text-orange-400 transition-colors duration-200 line-clamp-1 leading-snug">
        {task.title}
      </h4>
      ...
      <div className="flex items-center justify-between pt-3.5 border-t border-white/5">
        ...
        {task.assignedTo ? (
          <div 
            className="w-7 h-7 rounded-xl bg-orange-500 text-white font-extrabold flex items-center justify-center text-[10px] uppercase border border-white/10"
